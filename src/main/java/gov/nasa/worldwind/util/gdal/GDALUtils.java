...
public class GDALUtils {
    ...
    protected static boolean gdalPreLoadNativeLibrary(String folder, boolean allowLogErrors) {
        try {
            ...
            log.info(Logging.getMessage("generic.LibraryLoadedOK", GDAL_ALL_JNI));

            return true;
        } catch (Throwable t) {
            if (allowLogErrors) {
                log.debug(WWUtil.extractExceptionReason(t));
            }
        }
            ...
            if (srs.IsGeographic() == 1) {
                ...
                if (null != unit) {
                    ...
                    if ("meter".equals(unit) || "meters".equals(unit) || "metre".equals(unit) || "metres".equals(unit)) {
                        ...
                        log.warn(Logging.getMessage("generic.UnknownProjectionUnits", unit));
                    }
                }
