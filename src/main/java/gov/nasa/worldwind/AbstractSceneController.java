...
public abstract class AbstractSceneController extends WWObjectImpl implements SceneController {
    ...
    /**
     * The list of picked objects at the current pick point. This list is computed during each call to repaint. Initially <code>null</code>.
     */
    protected PickedObjectList lastPickedObjects;
    /**
     * The list of picked objects that intersect the current pick rectangle. This list is computed during each call to repaint. Initially <code>null</code>.
     */
     ...
     * {@link #doResolveTopPick(gov.nasa.worldwind.render.DrawContext, java.awt.Rectangle)}. This map is used only when a pick rectangle is specified. Initialized to a new
     * HashMap.
     */
    ...
    /**
     * Support class used to build the composite representation of surface objects as a list of SurfaceTiles. We keep a reference to the tile builder instance used to build tiles
     * because it acts as a cache key to the tiles and determines when the tiles must be updated. The tile builder does not retain any references the SurfaceObjects, so keeping a
     * reference to it does not leak memory.
     */
    ...
    public void reinitialize() {
        if (this.textRendererCache != null) this.textRendererCache.dispose();
     ...
     */
    public void dispose() {
        if (this.lastPickedObjects != null) this.lastPickedObjects.clear();
        this.lastPickedObjects = null;

        if (this.lastObjectsInPickRect != null) this.lastObjectsInPickRect.clear();
    ...
    public void setPerFrameStatisticsKeys(Set<String> keys) {
        ...
        for (String key : keys) {
            if (key != null) this.perFrameStatisticsKeys.add(key);
        }
    }
...
//    }
    ...
    public int repaint() {
        ...
        if (perfKeys == null) return dc.getRedrawRequested();
        ...
        if (perfKeys.contains(PerformanceStatistic.TEXTURE_CACHE) || perfKeys.contains(PerformanceStatistic.ALL)) {
            if (dc.getTextureCache() != null)
                this.dc.setPerFrameStatistic(PerformanceStatistic.TEXTURE_CACHE, "Texture Cache size (Kb)", this.dc.getTextureCache().getUsedCapacity() / 1000);
        }
        ...
        if (perfKeys.contains(PerformanceStatistic.JVM_HEAP) || perfKeys.contains(PerformanceStatistic.ALL)) {
            ...
            this.dc.setPerFrameStatistic(PerformanceStatistic.JVM_HEAP_USED, "JVM used memory (Kb)", (totalMemory - Runtime.getRuntime().freeMemory()) / 1000);
        }
...
//        checkGLErrors(dc);
    ...
    protected void applyView(DrawContext dc) {
        if (dc.getView() != null) dc.getView().apply(dc);
//
//        this.resetGroupingFilters();
    ...
    protected void createTerrain(DrawContext dc) {
        if (dc.getSurfaceGeometry() == null) {
            ...
            if (dc.getSurfaceGeometry() == null) {
                log.warn("generic.NoSurfaceGeometry");
            ...
            dc.setPerFrameStatistic(PerformanceStatistic.TERRAIN_TILE_COUNT, "Terrain Tiles", dc.getSurfaceGeometry().size());
        }
    }
    ...
    protected void preRender(DrawContext dc) {
        try {
            ...
            log.error(Logging.getMessage("BasicSceneController.ExceptionDuringPreRendering"), e);
        } finally {
    ...
    protected void pickTerrain(DrawContext dc) {
        if (dc.isPickingMode() && dc.getVisibleSector() != null && dc.getSurfaceGeometry() != null && dc.getSurfaceGeometry().size() > 0) {
            this.pickPoints.clear();
            if (dc.getPickPoint() != null) this.pickPoints.add(dc.getPickPoint());

            Point vpc = dc.getViewportCenterScreenPoint();
            if (vpc != null && dc.getViewportCenterPosition() == null) this.pickPoints.add(vpc);

            if (this.pickPoints.size() == 0) return;

            List<PickedObject> pickedObjects = dc.getSurfaceGeometry().pick(dc, this.pickPoints);
            if (pickedObjects == null || pickedObjects.size() == 0) return;

            for (PickedObject po : pickedObjects) {
                if (po == null) continue;
                if (po.getPickPoint().equals(dc.getPickPoint())) dc.addPickedObject(po);
                else if (po.getPickPoint().equals(vpc)) dc.setViewportCenterPosition((Position) po.getObject());
            }
        }
    }
        ...
        if (dc.getLayers() != null) {
            for (Layer layer : dc.getLayers()) {
                try {
                    ...
                    String message =
                            Logging.getMessage("SceneController.ExceptionWhilePickingInLayer", (layer != null ? layer.getClass().getName() : Logging.getMessage("term.unknown")));
                    log.error(message, e);
                    // Don't abort; continue on to the next layer.
                }
            }
    ...
    protected void resolveTopPick(DrawContext dc) {
        ...
        if (dc.getPickPoint() != null) this.doResolveTopPick(dc, dc.getPickPoint());

        // Resolve the top objects in the pick rectangle, if the pick rectangle is enabled.
        if (dc.getPickRectangle() != null && !dc.getPickRectangle().isEmpty()) this.doResolveTopPick(dc, dc.getPickRectangle());
    }
    ...
    protected void doResolveTopPick(DrawContext dc, Rectangle pickRect) {
        ...
        if (pol != null && pol.size() == 1) {
            ...
            for (PickedObject po : pol) {
                ...
                if (minAndMaxColorCodes == null) minAndMaxColorCodes = new int[]{colorCode, colorCode};
                else {
                    if (minAndMaxColorCodes[0] > colorCode) minAndMaxColorCodes[0] = colorCode;
                    if (minAndMaxColorCodes[1] < colorCode) minAndMaxColorCodes[1] = colorCode;
                }
            }
    ...
    protected void pick(DrawContext dc) {
        ...
        try {
            ...
            if (this.isDeferOrderedRendering()) return;
            ...
            log.error(Logging.getMessage("BasicSceneController.ExceptionDuringPick"), e);
        } finally {
    ...
    protected void doNonTerrainPick(DrawContext dc) {
        ...
        if (dc.getPickPoint() == null && (dc.getPickRectangle() == null || dc.getPickRectangle().isEmpty())) return;
        ...
        if (this.screenCreditController != null) this.screenCreditController.pick(dc, dc.getPickPoint());
...
//        dc.applyGroupingFilters();
    ...
    protected void doDeepPick(DrawContext dc) {
        ...
        this.lastObjectsInPickRect = this.mergePickedObjectLists(currentObjectsInPickRect, dc.getObjectsInPickRectangle());
    }
    ...
    protected void draw(DrawContext dc) {
        try {
            ...
            if (this.screenCreditController != null) this.screenCreditController.render(dc);
...
//            dc.applyGroupingFilters();
            ...
            while (dc.peekOrderedRenderables() != null) {
                try {
                    ...
                    log.warn(Logging.getMessage("BasicSceneController.ExceptionDuringRendering"), e);
                }
            }
            dc.setOrderedRenderingMode(false);

            // Draw the diagnostic displays.
            if (dc.getSurfaceGeometry() != null && dc.getModel() != null && (dc.getModel().isShowWireframeExterior() || dc.getModel().isShowWireframeInterior() || dc.getModel()
            ...
            log.error(Logging.getMessage("BasicSceneController.ExceptionDuringRendering"), e);
        }
    }

    /**
     * Called to check for openGL errors. This method includes a "round-trip" between the application and renderer, which is slow. Therefore, this method is excluded from the
     * "normal" render pass. It is here as a matter of convenience to developers, and is not part of the API.
     *
    ...
    protected void preRenderOrderedSurfaceRenderables(DrawContext dc) {
        if (dc.getOrderedSurfaceRenderables().isEmpty()) return;
        ...
        while (dc.getOrderedSurfaceRenderables().peek() != null) {
            try {
                ...
                if (or instanceof PreRenderable) ((PreRenderable) or).preRender(dc);
            } catch (Exception e) {
                log.warn(Logging.getMessage("BasicSceneController.ExceptionDuringPreRendering"), e);

                // Limit how many times we log a problem.
                if (++logCount > Logging.getMaxMessageRepeatCount()) break;
            }
        }
    ...
    /**
     * Builds a composite representation for all {@link gov.nasa.worldwind.render.SurfaceObject} instances in the draw context's ordered surface renderable queue. While building
     * the composite representation this invokes {@link gov.nasa.worldwind.render.SurfaceObject#render(gov.nasa.worldwind.render.DrawContext)} in ordered rendering mode. This does
     * nothing if the ordered surface renderable queue is empty, or if it does not contain any SurfaceObjects.
     * <p>
    ...
    /**
     * Causes the scene controller to draw the composite representation of all {@link gov.nasa.worldwind.render.SurfaceObject} instances in the draw context's ordered surface
     * renderable queue. This representation was built during the preRender phase. This does nothing if the ordered surface renderable queue is empty, or if it does not contain any
     * SurfaceObjects.
     *
    ...
    protected void drawCompositeSurfaceObjects(DrawContext dc) {
        ...
        int attributeMask = GL2.GL_COLOR_BUFFER_BIT   // For alpha test enable, blend enable, alpha func, blend func, blend ref.
