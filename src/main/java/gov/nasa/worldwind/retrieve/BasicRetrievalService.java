    ...
    implements RetrievalService, Thread.UncaughtExceptionHandler
{
    ...
    {
        log.debug(Logging.getMessage("BasicRetrievalService.UncaughtExceptionDuringRetrieval",
            thread.getName()));
    }
    ...
    private class RetrievalExecutor extends ThreadPoolExecutor
    {
        ...
        private RetrievalExecutor(int poolSize, int queueSize)
        {
                ...
                }, new ThreadPoolExecutor.DiscardPolicy() // abandon task when queue is full
            {
                ...
                public void rejectedExecution(Runnable runnable, ThreadPoolExecutor threadPoolExecutor)
                {
                    ...
                    log.debug(Logging.getMessage("BasicRetrievalService.ResourceRejected",
        ...
        protected void beforeExecute(Thread thread, Runnable runnable)
        {
            ...
            if (task.retriever.getBeginTime() - task.retriever.getSubmitTime() > limit)
            {
                ...
                log.debug(Logging.getMessage("BasicRetrievalService.CancellingTooOldRetrieval",
            ...
            {
                // Task is a duplicate
                log.debug(Logging.getMessage("BasicRetrievalService.CancellingDuplicateRetrieval",
                    task.getRetriever().getName()));
                task.cancel(true);
            }
        ...
        protected void afterExecute(Runnable runnable, Throwable throwable)
        {
            ...
            catch (java.util.concurrent.ExecutionException e)
            {
                ...
                {
                    log.debug(message + " " + e.getCause().getLocalizedMessage());
                }
            ...
            {
                log.debug(Logging.getMessage("BasicRetrievalService.RetrievalInterrupted",
                    task.getRetriever().getName()), e);
            }
            catch (java.util.concurrent.CancellationException e)
            {
                log.debug(Logging.getMessage("BasicRetrievalService.RetrievalCancelled",
                    task.getRetriever().getName()));
            }
            finally
            {
    ...
    public synchronized RetrievalFuture runRetriever(Retriever retriever, double priority)
    {
        ...
        if (!this.isAvailable())
        {
            log.debug(Logging.getMessage("BasicRetrievalService.ResourceRejected", retriever.getName()));
        }
